...
classification$Task <- case_match(
  classification$task,
...
classification$`Window Size` <- as.factor(classification$window_size)
classification$`Minimum Count` <- as.factor(classification$min_count)

p <- classification |>
  group_by(`Task`, `Model Group`, `Window Size`) |>
  summarise(
    mean = mean(`F1 Score`),
    min = min(`F1 Score`),
    max = max(`F1 Score`)
  ...
  scale_color_viridis(discrete = TRUE, option = "mako", end = 0.8) +
  facet_wrap(~Task, axes = "all", ncol = 2) +
  theme(legend.position = "top", plot.background = element_blank())

ggsave(
  "plots/classification/classification.png",
  p,
  width = 10,
  height = 15,
  units = "cm",
...
ggsave(
  "plots/classification/classification.pdf",
  p,
  width = 13,
  height = 15,
  units = "cm",
