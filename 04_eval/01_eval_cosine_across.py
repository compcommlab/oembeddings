...
def compare_model_groups(
    models: typing.Tuple[Path], lowercase: bool = False
) -> typing.List[dict]:
    results = []
    ...
    d_a = average_cosine_distance(models_a, words_random, vocabulary=shared_vocabulary)

    d_b = average_cosine_distance(models_b, words_random, vocabulary=shared_vocabulary)

    corr_random = [pearsonr(d_a[i], d_b[i])[0] for i in range(len(words_random))]
    corr_random = np.array(corr_random)

    random_results = dict(
        model_a_family=models[0].name,
    ...
    for cue, wordlist in CUES.items():
        ...
        d_a = average_cosine_distance(
            models_a, shared_wordlist, vocabulary=shared_vocabulary
        )

        d_b = average_cosine_distance(
            models_b, shared_wordlist, vocabulary=shared_vocabulary
        )
...
if __name__ == "__main__":
    arg_parser = ArgumentParser(
        description=(
            "Evaluate correlations across different parameter settings. "
    ...
    arg_parser.add_argument(
        "--debug",
        action="store_true",
        help="Debug flag: dry run the script (do not actually run the calculations)",
    )
    arg_parser.add_argument(
        "--threads",
        type=int,
    ...
    arg_parser.add_argument(
        "--model_a", type=str, help="Specify a directory with Model A group"
    )
    arg_parser.add_argument(
        "--model_b", type=str, help="Specify a directory with Model B group"
    )
    ...
    if input_args.model_a and input_args.model_b:
        ...
        assert model_a_dir.is_dir(), (
            f"Model A is not a directory! Model A path: {model_a_dir}"
        )
        model_b_dir = Path(input_args.model_b)
        assert model_b_dir.is_dir(), (
            f"Model B is not a directory! Model B path: {model_b_dir}"
        )
        assert model_a_dir.name != model_b_dir.name, (
            f"Both model families are the same! Can only calculate across correlation between different model families"
        )
        ...
        try:
            if not input_args.debug:
                results = compare_model_groups(
                    (model_a_dir, model_b_dir), lowercase=lowercase
                )
            else:
                results = []
        ...
        print(
            "Model combinations:",
            len(model_combinations) + len(model_combinations_lowercase),
        )
