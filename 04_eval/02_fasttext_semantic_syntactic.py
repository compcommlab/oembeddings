...
def test_most_similar(
    model: gensim.models.KeyedVectors, src: Path, label="most similar", topn=10
) -> dict:
    """
...
def test_most_similar_groups(
    model: gensim.models.KeyedVectors, src: Path, topn=10
) -> typing.List[dict]:
    """
    Tests given model to most similar word.
    ...
    with open(src) as groups_fp:
        ...
        # calculate result
        correct_matches = (
            round(num_right / float(num_questions) * 100, 1)
            if num_questions > 0
            else 0.0
        )
        topn_matches = (
            round(num_topn / float(num_questions) * 100, 1)
            if num_questions > 0
            else 0.0
        )
        coverage = (
...
def test_doesnt_fit(model: gensim.models.KeyedVectors, src: Path) -> dict:
    """
...
def evaluate_model(
    model_path: str, topn: int, umlauts=False, lowercase=False, meta: dict = None
) -> None:
    ...
    most_similar_results = test_most_similar_groups(
        model, str(TARGET_SYN) + ".lower" if lowercase else TARGET_SYN, args.topn
    )
    ...
    results_destination = MOST_SIMILAR_DIR / Path(model_path).name.replace(
        ".vec", "_most_similar.json"
    )
    ...
    result = test_most_similar(
        model,
        str(TARGET_SEM_OP) + ".lower" if lowercase else TARGET_SEM_OP,
        "opposite",
        topn,
    )
    if meta:
    ...
    results_destination = OPPOSITE_DIR / Path(model_path).name.replace(
        ".vec", "_opposite.json"
    )
    with open(results_destination, "w") as f:
    ...
    result = test_most_similar(
        model,
        str(TARGET_SEM_BM) + ".lower" if lowercase else TARGET_SEM_BM,
        "best match",
        topn,
    )
    if meta:
    ...
    results_destination = BEST_MATCH_DIR / Path(model_path).name.replace(
        ".vec", "_best_match.json"
    )
    with open(results_destination, "w") as f:
        json.dump(result, f, indent=True)

    result = test_doesnt_fit(
        model, str(TARGET_SEM_DF) + ".lower" if lowercase else TARGET_SEM_DF
    )
    if meta:
        result["name"] = meta["name"]
        result["parameter_string"] = meta["parameter_string"]

    results_destination = WORD_INTRUSION_DIR / Path(model_path).name.replace(
        ".vec", "_word_intrusion.json"
    )
    with open(results_destination, "w") as f:
        json.dump(result, f, indent=True)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Script for creating testsets and evaluating word vector models. If no "model" parameter is specified, the script will evaluate all models in the tmp_models directory.'
    )
    ...
    parser.add_argument(
        ...
        help="glob pattern for several models to evaluate",
    )
    parser.add_argument(
        "-u",
        "--umlauts",
    ...
    if args.model:
        ...
        logging.basicConfig(
            filename="syntactic_evaluation_results.log",
            format="%(asctime)s : %(message)s",
            level=logging.INFO,
        )
    ...
    if not all(
        [
            TARGET_SYN.exists(),
            TARGET_SEM_OP.exists(),
            TARGET_SEM_BM.exists(),
            TARGET_SEM_DF.exists(),
        ]
    ):
    ...
    if args.model:
        evaluate_model(
            args.model,
            args.topn,
            umlauts=args.umlauts,
            lowercase="lower" in args.model or "wiki" in args.model,
        )
    else:
        model_dir = get_data_dir()
        models_meta = [json.load(f.open()) for f in model_dir.glob(args.glob)]
        ...
        for model in models_meta:
            evaluate_model(
                ...
                args.topn,
                meta=model,
                lowercase="lower" in model["parameter_string"],
            )
